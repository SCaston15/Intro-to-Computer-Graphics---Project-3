...
/**
 * Return the magnitude (i.e. length) of this Vec2 
 * @return {Number}
 */
Vec2.prototype.mag = function ()
{
   return Math.sqrt(this.x * this.x + this.y * this.y);    return 0;
};
...
function barycentric (p0, p1, p2, p)
{
	...
	var firstVec = new Vec2(); var secondVec = new Vec2(); var thirdVec = new Vec2(); var point = new Vec2();
    ...
    secondVec.sub(firstVec); thirdVec.sub(firstVec); point.sub(firstVec);
    var v0 = secondVec, v1 = thirdVec, v2 = point;
...
{
	var matrix1 = new Mat2();
	var firstVec = new Vec2();
	firstVec.x = p1[0];
	firstVec.y = p1[1];
	console.log(firstVec);
	var y = firstVec.mag();
	var secondVec = new Vec2();
	secondVec.x = p0[0];
	secondVec.y = p0[1];
	firstVec.sub(secondVec);
	matrix1.set(0, 0, firstVec.x);
	matrix1.set(0, 1, firstVec.y);
	var point = new Vec2();
	point.x = p[0];
	point.y = p[1];
	secondVec.sub(point);
	matrix1.set(1, 0, secondVec.x);
	matrix1.set(1, 1, secondVec.y);
	var x = Math.abs(matrix1.det() / y);
    return x;
}
